...
class BiReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_undue_amount(self, move_state, account_type, date_from, partner_ids, company_ids):
        ...
        _logger.info("====================================")
        _logger.info(query)
        _logger.info(tuple(move_state))
        _logger.info(tuple(account_type))
        _logger.info(date_from)
        _logger.info(tuple(partner_ids))
        _logger.info(date_from)
        _logger.info(tuple(company_ids))
        _logger.info("====================================")

        return self.cr.fetchall()

    def _get_partners(self, reconciliation_clause, arg_list):
        query = '''
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        query = 'SELECT debit_move_id, credit_move_id FROM account_partial_reconcile where create_date > %s'
        cr.execute(query, (date_from,))
        ...
        partners = self._get_partners(reconciliation_clause, arg_list)
        # put a total of 0
        for i in range(7):
            total.append(0)
