...
class CashbookWizard(models.TransientModel):
    ...
    cash_account_id = fields.Many2one('account.journal',required=True, string='Cash Account', domain="[('type', '=', 'cash')]")   
    ...
    def print(self):
        ...
        if not report_data:
            raise ValidationError('Cashbook is empty')

        if self._context.get('report_type') == 'excel':
            return self.env['cashbook.excel'].print(report_data,data)
        else:
    ...
    def _get_report_data(self):
        ...
        params = (
                    self.cash_account_id.id,self.cash_account_id.id, self.start_date, 
                    self.cash_account_id.id,self.cash_account_id.id, self.start_date, self.end_date,
                    self.cash_account_id.id,self.cash_account_id.id, self.start_date, self.end_date,
                    self.cash_account_id.id,self.cash_account_id.id, self.start_date, self.end_date,
                    self.cash_account_id.id,self.cash_account_id.id, self.start_date,
                    self.cash_account_id.id,self.cash_account_id.id, self.start_date, self.end_date,
                    self.cash_account_id.id,self.cash_account_id.id, self.end_date
                )
